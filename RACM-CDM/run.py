...
def IMDER_run(
        ...
        model_save_dir="/root/autodl-tmp/pt", res_save_dir="/root/autodl-tmp/result", log_dir="/root/autodl-tmp/log",
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    # model.load_state_dict(torch.load('pt/imder-{}.pth'.format(args.dataset_name)), strict=False)
    # model.load_state_dict(torch.load('/root/autodl-tmp/pt/imder-mosi-besttest.pth'.format(args.dataset_name)), strict=False)
