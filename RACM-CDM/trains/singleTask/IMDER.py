...
class IMDER():
    """
    初始化时，会根据 args 参数中的 train_mode 来确定损失函数是 L1Loss 还是 CrossEntropyLoss。
    初始化评价指标 MetricsTop，这可能是一个自定义的类，用于计算和返回训练和测试过程中的各种评价指标。
    """
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        while True:
            ...
            if isBetter:
                ...
                bestvalid_acc = val_results['Acc_2']
                # save model
                torch.save(model.cpu().state_dict(), self.args.model_save_path)
                model.to(self.args.device)
            ## save best test model
            if besttest_acc < test_results['Acc_2']:
                besttest_acc = test_results['Acc_2']
                testmodel_save_path = '/root/autodl-tmp/pt/imder-mosi-besttest.pth'
                torch.save(model.cpu().state_dict(), testmodel_save_path)
                model.to(self.args.device)
            logger.info(
                f"Best valid:({bestvalid_acc})"
                f"Best test:({besttest_acc})"
            )
            # epoch results
            if return_epoch_results:
